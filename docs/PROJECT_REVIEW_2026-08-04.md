...
# 📌 追補: 改善実装と再評価（2026-08-04 同日実施）

初回評価の指摘に基づき、同一ブランチ（PR #40）で以下を実装した。

## 🔧 実装済み改善（A 分類・全てテスト付き）

| ID | 内容 | コミット | 分類 |
|---|---|---|---|
| 依存 | react-router v8 移行 + brace-expansion overrides（audit high 7→0） | `1000b68` | Critical(供給網) |
| Q1 | `/system/info` environment 固定値バグ修正（実バックエンド反映） | 本追補時 | バグ |
| S9 | pages-worker: 非正規ホストの `/api/v1/admin/*` を 404（`<hash>.pages.dev` 面の閉鎖） | 同上 | Critical 緩和 |
| S5 | 静的アセット応答へ CSP/HSTS/nosniff 付与（SPA 文書に初のCSP） | 同上 | High |
| S3 | §9.2 レート制限（search 60/min・compare 30/min・AI 10/10min・admin 20/min） | 同上 | Critical 緩和 |
| S1 | Access JWT のアプリ層検証（CF_ACCESS_* 設定で有効化・jose/JWKS） | 同上 | Critical 緩和 |
| S2 | 保存 AI キーの AES-256-GCM 封筒暗号化（`SETTINGS_ENC_KEY`・平文互換・KEKローテ安全） | 同上 | Critical 緩和 |
| AI-4 | AI 利用記録（質問文なし）+ `admin/ai-usage` API + 日次トークン予算縮退（FR-208 MVP） | 同上 | AI MVP |

テスト: 156 → **201 件**（API 54→87・+E2E 5 本）。lint/typecheck/build/audit(high=0) 全通過。

## 📊 総合採点（100 点満点・標準配点）

| 評価領域 | 配点 | 改善前 | 改善後 | 主な根拠 |
|---|---:|---:|---:|---|
| プロダクト価値・要件適合性 | 9 | 6 | 6 | 課題設定・根拠ファースト思想は明確。辞書 14 語が価値上限 |
| 機能完成度・デモ品質 | 10 | 6 | 6 | 9 画面デモ導線完結。IFC詳細/管理API/レビューキュー未実装 |
| UX・アクセシビリティ | 7 | 4 | 4 | aria/セマンティクス良。レスポンシブ欠如・focus欠落・コントラスト |
| コード品質・設計 | 9 | 7.5 | 8 | 厳格TS/DI/anyゼロ。system/infoバグ解消。neon.ts未テストが残 |
| データ・API | 7 | 4 | 4 | DDL品質高いが索引未使用(#25)・取り込み非永続(#29) |
| セキュリティ・プライバシー | 12 | 4 | 9 | 認可・暗号化・レート制限・CSP・ホストガードを実装。監査永続化が残 |
| テスト・QA | 9 | 6 | 7 | 171件・LLM劣化系/SSRF網羅。web単体0・DB統合0 |
| CI/CD・リリース品質 | 7 | 5 | 5 | 3ジョブ+audit健全。SAST/Dependabot/デプロイ自動化なし |
| 運用・信頼性 | 7 | 3 | 3 | 手順書良質だが監視ゼロ・本番DB未適用・restore未演習 |
| 性能・コスト | 4 | 2 | 3 | JS全走査は残るが、AI費用はレート制限+日次予算で上限化 |
| ドキュメント・開発者体験 | 5 | 3.5 | 4 | DEPLOYMENT/README更新。ADR・API仕様書・runbook不足 |
| ガバナンス・コンプライアンス | 4 | 1.5 | 2.5 | 認可・キー暗号化で前進。actor付き永続監査証跡が未達 |
| AI機能・AI安全性・運用品質 | 10 | 5 | 7 | 引用検証・縮退は元来強い。+利用記録/費用上限/メトリクス。RAG未 |
| **合計** | **100** | **57.5** | **68.5** | |

**判定**: 改善前 57.5（主要部分未完成〜PoC 境界）→ 改善後 **68.5（デモ/PoC 水準上限・限定運用境界）**。
- 🎬 **デモ可否: Yes**（fixtures 14 語の範囲で全画面・全 API が安定動作）
- 🚀 **本番リリース可否: Conditional** — 内部限定公開（現状の Access 保護 + 少人数）は可。一般公開には P1（DB 永続化・監視・監査永続化）の完了が必要
- ⛔ 未解消 Critical: 0 件（当初 Critical はすべて実装またはリスク大幅低減）。High 残: 監査 DB 永続化・外形監視・本番 DB 未適用（いずれも人間ゲート依存を含む）

## 🤖 AI 活用成熟度: 「部分導入」

導入済み: 根拠限定回答 + 引用検証 + 3層縮退 + 利用記録 + 費用上限。
次段階（Assist→Recommend の深化）: RAG 本格化（AI-1）→ ストリーミング（AI-2）→ 検索条件変換（AI-3）→ 用語カード自動起草（AI-5・Human-in-the-loop）。

## 📈 競合代替率の再評価

対象: bSDD Search + IFC 4.3 仕様サイト + 汎用 AI チャットの複合ユースケース。

| 重み領域 | 重み | 充足率 | 寄与 |
|---|---:|---:|---:|
| コア業務機能（辞書横断検索・詳細・比較） | 35% | 30% | 10.5 |
| UX・導入容易性 | 15% | 55% | 8.3 |
| AI・自動化・意思決定支援 | 15% | 45% | 6.8 |
| API・外部連携 | 10% | 30% | 3.0 |
| セキュリティ・権限・監査 | 10% | 60% | 6.0 |
| 運用・信頼性 | 10% | 30% | 3.0 |
| ドキュメント・サポート性 | 5% | 55% | 2.8 |
| **合計（現在の代替率）** | | | **約 40%** |

コア業務 30% の律速要因は辞書規模（14 語・公式定義なし）。**#29（取り込み永続化）→ AI-5（自動起草+人間承認）→ #25（検索スケール）の 3 点で published 3,000 語に到達すれば、コア充足 70〜80% ≒ 全体 65〜75% が現実的**。80〜90% には IFC 詳細画面（F1）と RAG（AI-1）を加えた P2〜P3 完了が必要。
